-- AI培训陪练系统初始数据插入脚本（修正JSON格式）
-- 使用现有表结构：training_scripts, customer_personas

-- 1. 首先检查并创建customer_personas表（如果不存在）
...
-- 3. 插入培训剧本数据到training_scripts表（使用现有字段结构）
...
-- 4. 插入AI提示词配置到ai_prompt_configs表（检查字段是否存在）
...
('ai_model_settings', '{\"provider\": \"deepseek\", \"temperature\": 0.7, \"max_tokens\": 2000}', 'AI模型配置'),
('evaluation_weights', '{\"goal_achievement\": 40, \"professionalism\": 25, \"efficiency\": 15, \"adaptability\": 10, \"customer_satisfaction\": 10}', '评估权重配置'),
('session_settings', '{\"max_rounds\": 5, \"response_delay\": \"normal\", \"auto_save\": true}', '会话设置');

SELECT '培训陪练系统初始数据插入完成（使用现有表结构）！' as status;