...
- **用途**: 识别微信聊天截图中的文字
- **费用**: 按调用量计费，有免费额度
- **需要配置**: API Key + Endpoint ID

**获取步骤**:
1. 访问火山方舟控制台: https://console.volcengine.com/ark
2. 注册/登录字节跳动火山引擎账号
3. 创建 API Key:
   - 在左侧菜单找到"密钥管理"或"API Key"
   - 点击"创建API Key"
   - 复制生成的API Key（格式：字母数字混合）
4. 创建推理接入点（Endpoint）:
   - 在左侧菜单找到"推理"或"在线推理"
   - 点击"创建推理接入点"
   - 选择模型：**doubao-vision-pro** 或 **doubao-1.5-vision-pro**
   - 创建完成后，复制 **Endpoint ID**（格式：`ep-xxxxxxxxxxxxx`）
5. 妥善保存两个值:
   - `DOUBAO_API_KEY`: 你的API Key
   - `DOUBAO_ENDPOINT_ID`: ep-xxxxxxxxxxxxx

**⚠️ 重要说明**:
- 火山方舟使用的是 **Endpoint ID**，不是模型名称
- 如果提示"模型不存在"，请确认已正确创建推理接入点
- Endpoint ID 格式必须是 `ep-` 开头
