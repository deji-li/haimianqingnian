...
Write-Host "Insert Test Data" -ForegroundColor Cyan
Write-Host "================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "[1/2] Inserting test data..." -ForegroundColor Yellow

# Step 1: Insert roles first (no dependencies)
Write-Host "  Inserting roles..." -ForegroundColor Gray
mysql -uroot --default-character-set=utf8mb4 $database -e @"
...
Write-Host "  Inserting permissions..." -ForegroundColor Gray
mysql -uroot --default-character-set=utf8mb4 $database -e @"
...
Write-Host "  Inserting departments..." -ForegroundColor Gray
mysql -uroot --default-character-set=utf8mb4 $database -e @"
...
# Step 4: Insert campuses
Write-Host "  Inserting campuses..." -ForegroundColor Gray
mysql -uroot --default-character-set=utf8mb4 $database -e @"
...
Write-Host "  Getting role IDs..." -ForegroundColor Gray
$adminRoleId = mysql -uroot -N -e "USE $database; SELECT id FROM roles WHERE code='admin' LIMIT 1;" 2>&1
$salesRoleId = mysql -uroot -N -e "USE $database; SELECT id FROM roles WHERE code='sales' LIMIT 1;" 2>&1
$salesDeptId = mysql -uroot -N -e "USE $database; SELECT id FROM department WHERE departmentName='销售部' LIMIT 1;" 2>&1

# Step 6: Insert users with actual role IDs
Write-Host "  Inserting users..." -ForegroundColor Gray
if ($adminRoleId -is [array]) { $adminRoleId = $adminRoleId[0] }
if ($salesRoleId -is [array]) { $salesRoleId = $salesRoleId[0] }
if ($salesDeptId -is [array]) { $salesDeptId = $salesDeptId[0] }
$adminRoleId = "$adminRoleId".Trim()
$salesRoleId = "$salesRoleId".Trim()
$salesDeptId = "$salesDeptId".Trim()

# Insert admin user (password: admin123)
mysql -uroot --default-character-set=utf8mb4 $database -e @"
INSERT INTO users (username, password, realName, phone, email, roleId, status, createTime, updateTime) VALUES
('admin', '\$2b\$10\$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lhub', '系统管理员', '13800138000', 'admin@example.com', $adminRoleId, 1, NOW(), NOW());
"@ 2>&1 | Out-Null

# Insert sales users
mysql -uroot --default-character-set=utf8mb4 $database -e @"
INSERT INTO users (username, password, realName, phone, email, roleId, departmentId, status, createTime, updateTime) VALUES
('sales001', '\$2b\$10\$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lhub', '张三', '13900001001', 'zhangsan@example.com', $salesRoleId, $salesDeptId, 1, NOW(), NOW()),
('sales002', '\$2b\$10\$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lhub', '李四', '13900001002', 'lisi@example.com', $salesRoleId, $salesDeptId, 1, NOW(), NOW());
"@ 2>&1 | Out-Null

# Step 7: Insert customers
Write-Host "  Inserting customers..." -ForegroundColor Gray
mysql -uroot --default-character-set=utf8mb4 $database -e @"
...
# Step 8: Insert knowledge base
Write-Host "  Inserting knowledge base..." -ForegroundColor Gray
mysql -uroot --default-character-set=utf8mb4 $database -e @"
...
# Step 9: Insert dictionary data
Write-Host "  Inserting dictionary data..." -ForegroundColor Gray
mysql -uroot --default-character-set=utf8mb4 $database -e @"
...
# Verify data
Write-Host "[2/2] Verifying inserted data..." -ForegroundColor Yellow
$userCount = mysql -uroot -N -e "USE $database; SELECT COUNT(*) FROM users;" 2>&1
$roleCount = mysql -uroot -N -e "USE $database; SELECT COUNT(*) FROM roles;" 2>&1
$customerCount = mysql -uroot -N -e "USE $database; SELECT COUNT(*) FROM customers;" 2>&1
$kbCount = mysql -uroot -N -e "USE $database; SELECT COUNT(*) FROM enterprise_knowledge_base;" 2>&1
$deptCount = mysql -uroot -N -e "USE $database; SELECT COUNT(*) FROM department;" 2>&1
$campusCount = mysql -uroot -N -e "USE $database; SELECT COUNT(*) FROM campus;" 2>&1
$dictCount = mysql -uroot -N -e "USE $database; SELECT COUNT(*) FROM dictionary;" 2>&1

Write-Host "  Users: $($userCount.Trim())" -ForegroundColor White
Write-Host "  Roles: $($roleCount.Trim())" -ForegroundColor White
Write-Host "  Customers: $($customerCount.Trim())" -ForegroundColor White
Write-Host "  Knowledge Base: $($kbCount.Trim())" -ForegroundColor White
Write-Host "  Departments: $($deptCount.Trim())" -ForegroundColor White
Write-Host "  Campuses: $($campusCount.Trim())" -ForegroundColor White
Write-Host "  Dictionary: $($dictCount.Trim())" -ForegroundColor White
